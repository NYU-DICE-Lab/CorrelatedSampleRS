...
singularity exec --nv \
	...
	/bin/bash -c "source /ext3/env.sh; python infer_certify_pretrained_salman_uncorrelated.py imagenet -dpath /scratch/aaj458/data/ImageNet/val/ -mp salman_models/pretrained_models/imagenet/PGD_1step/imagenet/eps_2048/resnet50/noise_$SIGMA/checkpoint.pth.tar -mt resnet50 -ni 500 -ps $PATCH_SIZE -pstr $PATCH_STRIDE -sigma $SIGMA --N0 100 --N 10000 --patch -o cifar10_SOTA_contd/certify_results_salman_patchsmooth_smoothmax_randompatches_$MAXPATCHES/ --batch 400 -rm $RMODE -ns 36 -rp -np $MAXPATCHES --normalize -si $START_IDX"
