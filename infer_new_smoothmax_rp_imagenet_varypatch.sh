...
singularity exec --nv \
	--overlay /scratch/aaj458/singularity_containers/my_pytorch.ext3:ro \
	/scratch/aaj458/singularity_containers/cuda11.1.1-cudnn8-devel-ubuntu20.04.sif \
	/bin/bash -c "source /ext3/env.sh; python infer_certify_pretrained_salman_uncorrelated.py imagenet -dpath /scratch/aaj458/data/ImageNet/val/ -mp salman_models/pretrained_models/imagenet/PGD_1step/imagenet/eps_512/resnet50/noise_$SIGMA/checkpoint.pth.tar -mt resnet50 -ni 500 -ps $PATCH_SIZE -pstr $PATCH_STRIDE -sigma $SIGMA --N0 100 --N 100000 --patch -o imagenet_SOTA_varypatch/certify_results_salman_patchsmooth_smoothmean_randompatches_$MAXPATCHES_$NEWSIZE/ --batch 128 -rm $RMODE -ns $NEWSIZE -rp -np $MAXPATCHES -si $START_IDX"
